...
pub trait KeycloakManagement {
    ...
    fn query_users(
        &self,
        request: &UsersQuery,
    ) -> impl Future<Output = Result<Vec<UserResponse>, AppErr>> + Send;
    ...
    ) -> impl Future<Output = Result<Vec<ClientResponse>, AppErr>> + Send;
    ...
    fn query_role(
        &self,
        request: &RoleQuery,
    ) -> impl Future<Output = Result<RoleResponse, AppErr>> + Send;
